...
private:
    ...
    int splitSize = 0;
private:
    static void* threadFunction(void *input) {
...
public:
    ...
    Matrix<Matrix<int> > multiplySplitAB(Matrix<Matrix<int> > A, Matrix<Matrix<int> > B) {
        ...
        for (size_t i = 0; i < A.size(); ++i)
            for (size_t j = 0; j < B.front().size(); ++j) {
                std::vector<pthread_t*> thArr(B.size());
                std::vector<Matrix<int>* > blocks(B.size());
                t_lock.lock();
                for (size_t k = 0; k < B.size(); ++k) {
                    thArr[k]=new pthread_t;
                    ...
                    pthread_create(thArr[k],NULL,threadFunction,mPair);
                }
                for (size_t k = 0; k < B.size(); ++k) {
                    pthread_join(*thArr[k],NULL);
                    if (C[i][j].empty())
                        C[i][j] = *(blocks[k]);
                    else
